...
parser = argparse.ArgumentParser(
    ...
    description="Train or load a GRU4Rec model & measure recall and MRR on the specified test set(s).",
)
parser.add_argument(
    "path",
    metavar="PATH",
    type=str,
    help="Path to the training data (TAB separated file (.tsv or .txt) or pickled pandas.DataFrame object (.pickle)) (if the --load_model parameter is NOT provided) or to the serialized model (if the --load_model parameter is provided).",
)
parser.add_argument(
    "-ps",
    "--parameter_string",
    metavar="PARAM_STRING",
    type=str,
    help="Training parameters provided as a single parameter string. The format of the string is `param_name1=param_value1,param_name2=param_value2...`, e.g.: `loss=bpr-max,layers=100,constrained_embedding=True`. Boolean training parameters should be either True or False; parameters that can take a list should use / as the separator (e.g. layers=200/200). Mutually exclusive with the -pf (--parameter_file) and the -l (--load_model) arguments and one of the three must be provided.",
)
parser.add_argument(
    "-pf",
    "--parameter_file",
    metavar="PARAM_PATH",
    type=str,
    help="Alternatively, training parameters can be set using a config file specified in this argument. The config file must contain a single OrderedDict named `gru4rec_params`. The parameters must have the appropriate type (e.g. layers = [100]). Mutually exclusive with the -ps (--parameter_string) and the -l (--load_model) arguments and one of the three must be provided.",
)
parser.add_argument(
    "-l",
    "--load_model",
    action="store_true",
    help="Load an already trained model instead of training a model. Mutually exclusive with the -ps (--parameter_string) and the -pf (--parameter_file) arguments and one of the three must be provided.",
)
parser.add_argument(
    "-s",
    "--save_model",
    metavar="MODEL_PATH",
    type=str,
    help="Save the trained model to the MODEL_PATH. (Default: don't save model)",
)
parser.add_argument(
    "-t",
    "--test",
    ...
    type=str,
    nargs="+",
    help="Path to the test data set(s) located at TEST_PATH. Multiple test sets can be provided (separate with spaces). (Default: don't evaluate the model)",
)
parser.add_argument(
    "-m",
    "--measure",
    ...
    nargs="+",
    default=[20],
    help="Measure recall & MRR at the defined recommendation list length(s). Multiple values can be provided. (Default: 20)",
)
parser.add_argument(
    "-e",
    "--eval_type",
    ...
    default="standard",
    help="Sets how to handle if multiple items in the ranked list have the same prediction score (which is usually due to saturation or an error). See the documentation of batch_eval() in evaluation.py for further details. (Default: standard)",
)
parser.add_argument(
    "-ss",
    "--sample_store_size",
...
parser.add_argument(
    ...
    metavar="GRFILE",
    type=str,
    default="gru4rec_pytorch",
    help="Name of the file containing the GRU4Rec class. Can be used to select different varaiants. (Default: gru4rec_pytorch)",
)
parser.add_argument(
    "-d",
    "--device",
    ...
    type=str,
    default="cuda:0",
    help="Device used for computations (default: cuda:0).",
)
parser.add_argument(
    "-ik",
    "--item_key",
    ...
    type=str,
    default="ItemId",
    help="Column name corresponding to the item IDs (detault: ItemId).",
)
parser.add_argument(
    "-sk",
    "--session_key",
    ...
    type=str,
    default="SessionId",
    help="Column name corresponding to the session IDs (default: SessionId).",
)
parser.add_argument(
    "-tk",
    "--time_key",
    ...
    type=str,
    default="Time",
    help="Column name corresponding to the timestamp (default: Time).",
)
parser.add_argument(
    "-pm",
    "--primary_metric",
    ...
    choices=["recall", "mrr"],
    default="recall",
    help="Set primary metric, recall or mrr (e.g. for paropt). (Default: recall)",
)
parser.add_argument(
    "-lpm",
    "--log_primary_metric",
    action="store_true",
    help="If provided, evaluation will log the value of the primary metric at the end of the run. Only works with one test file and list length.",
)
...
if (args.parameter_string is not None) + (args.parameter_file is not None) + (
    args.load_model
) != 1:
    print(
...
if args.load_model:
    print(
        'Loading trained model from file: {} (to device "{}")'.format(
            args.path, args.device
        )
    )
    ...
    if args.parameter_file:
        ...
        spec = importlib.util.spec_from_file_location(
            param_file.split(".py")[0], os.path.abspath(args.parameter_file)
        )
    ...
    if args.parameter_string:
        gru4rec_params = OrderedDict(
            [x.split("=") for x in args.parameter_string.split(",")]
        )
...
if args.test is not None:
    if args.primary_metric.lower() == "recall":
        ...
        raise RuntimeError(
            "Invalid value `{}` for `primary_metric` parameter".format(
                args.primary_metric
            )
        )
    for test_file in args.test:
        ...
        for c in args.measure:
            print(
                f"HR@{c}: {res[0][c]:.6f} | MRR@{c}: {res[1][c]:.6f} | NDCG@{c}: {res[2][c]:.6f}"
            )
